    ...
    <div class="product-detail-info">
      ...
      <p class="product-detail-price">
        ...
        <?php if ($product['old_price']): ?>
          <span style="text-decoration:line-through;color:#999;font-size:0.9rem;margin-left:8px;">$<?= number_format($product['old_price'], 2) ?></span>
          <span class="discount-badge" style="margin-left:6px;">-<?= $discount_pct ?>%</span>
        <?php endif; ?>
      </p>
      <div class="product-detail-stars"><?= $stars ?> <span style="font-size:0.85rem;color:#6B6B6B;margin-left:6px;"><?= number_format($product['rating'], 1) ?>/5</span></div>
      <p class="product-detail-desc">
        <?= htmlspecialchars($product['desc']) ?>
      </p>
      <p class="product-detail-allergy">
      ...
      <div class="product-detail-actions">
        ...
        <button class="product-detail-btn-wishlist <?= $is_wishlisted ? 'wishlisted-detail' : '' ?>" id="wishlistBtn" onclick="toggleWishlistDetail(<?= $id ?>)">
          <i class="<?= $is_wishlisted ? 'fas' : 'far' ?> fa-heart"></i>
          <?= $is_wishlisted ? 'IN WISHLIST' : 'ADD TO WISH LIST' ?>
        </button>
        <button class="product-detail-btn-cart" style="background:#5C6B3A;border-color:#5C6B3A;" onclick="buyNow(<?= $id ?>)">
